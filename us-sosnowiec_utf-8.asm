...
random PROC
     	...
     	ENDP
;------------------------------------------------------------------------------
; procedura $D016, to opis dla tych, ktorzy wiedza za co odpowiada ta komorka pamieci w C64 ;)
;------------------------------------------------------------------------------
scroll PROC
        ;dec lwait
	    ;jnz @scexit
...
@nie_spacja:
	    ...
	    add bl,poz_fonta        ;dodaj bierzaca pozycje z ktorej kopiujemy kolumne danego znaku
	    mov di,320*scroll_y+319 ;policz miejsce w ktore bedziemy kopiowali bierz. kolumne
...
@spacja:                        
        ...
        jmp @scexit
@sc4:
	    mov poz_fonta,0         ;bierzaca kopiowana kolumna byla pusta, czyli nastepny znak
	    inc wsk_tekstu
@scexit:
	    ret
	    ENDP
